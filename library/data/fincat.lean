...
namespace fin
  ...
  theorem zero.ext {C : fin 0 → Type} {f g : Π i : fin 0, C i} : f = g := funext zero.rec
  ...
  theorem one.ext {C : fin 1 → Type} {H : C (zero 0)} {f g : Π i : fin 1, C i} :
    f (zero 0) = g (zero 0) → f = g :=
  ...
  theorem add.ext {m n : nat} {C : fin (m + n) → Type} {f g : Π k : fin (m + n), C k} :
    (∀ i : fin m, f (add.inl i) = g (add.inl i)) →
    (∀ j : fin n, f (add.inr j) = g (add.inr j)) →
    f = g :=
    take Hinl Hinr, funext (add.rec Hinl Hinr)

  theorem add.eq_rec_inl {m n : nat} {C : fin (m + n) → Type}
  ...
  theorem add.eq_rec_inr {m n : nat} {C : fin (m + n) → Type}
    {F : Π i : fin m, C (add.inl i)} {G : Π j : fin n, C (add.inr j)} :
  ...
  lemma mul.mk_is_lt {m n : nat} {i j : nat} : i < m → j < n → i * n + j < m * n :=
  take hi hj,
  let m' := nat.pred m in
  ...
  theorem mul.eq_mk_pr1_pr2 {m n : nat} : ∀ (k : fin (m * n)), k = mul.mk (mul.pr1 k) (mul.pr2 k)
  | (mk v h) := eq_of_veq (eq_div_mul_add_mod v n)

  theorem mul.eq_pr1_mk {m n : nat} : ∀ (i : fin m) (j : fin n), i = mul.pr1 (mul.mk i j)
  | (mk vi hi) (mk vj hj) :=
  ...
  theorem mul.eq_pr2_mk {m n : nat} : ∀ (i : fin m) (j : fin n), j = mul.pr2 (mul.mk i j)
  | (mk vi hi) (mk vj hj) :=
    eq_of_veq (calc
  ...
  theorem mul.ext {m n : nat} {C : fin (m * n) → Type} {f g : Π k : fin (m * n), C k} :
    (∀ (i : fin m) (j : fin n), f (mul.mk i j) = g (mul.mk i j)) → f = g :=
  take Hmk, funext (mul.rec Hmk)

  theorem mul.eq_rec_mk {m n : nat} {C : fin (m * n) → Type}
    {F : Π (i : fin m) (j : fin n), C (mul.mk i j)} :
  ...
  theorem exp.eq_ev_fn {m n : nat} : ∀ f : fin n → fin m, f = exp.ev (exp.fn f) :=
  nat.induction_on n
    (take f, zero.ext)
  ...
  theorem exp.eq_fn_ev {m n : nat} : ∀ k : fin (m ^ n), k = exp.fn (exp.ev k) :=
  nat.induction_on n
  ...
  theorem exp.ext {m n : nat} {C : (fin n → fin m) → Type} {f g : Π h : fin n → fin m, C h} :
    (Π i : fin (m ^ n), f (exp.ev i) = g (exp.ev i)) → f = g :=
  take Hev, funext (exp.rec Hev)

  theorem exp.eq_rec_ev {m n : nat} {C : (fin n → fin m) → Type}
